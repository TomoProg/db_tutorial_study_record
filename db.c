...
typedef struct {
	StatementType type;
	Row row_to_insert; // insertに使うための行データ
} Statement;

// 構造体の属性のサイズを取得する関数形式マクロ
// Struct, Attributeは仮引数で、実際に使われるときに指定された内容で置換される
...
const uint32_t PAGE_SIZE = 4096; 									// 1ページのサイズ（ほとんどのコンピュータアーキテクチャの仮想メモリシステムで使われるページと同じサイズ）
#define TABLE_MAX_PAGES 100 										// テーブルの最大ページ数
const uint32_t ROWS_PER_PAGE = PAGE_SIZE / ROW_SIZE;				// 1ページあたりの行数
const uint32_t TABLE_MAX_ROWS = ROWS_PER_PAGE * TABLE_MAX_PAGES;	// テーブルの最大の行数

// テーブルはページの集合
// 1ページあたりに格納できる行数も決まっている
// こんなイメージ
// *-Table------------------------------------*
// |                                          |
// | *-Page1--------------------------------* |
...
PrepareResult prepare_insert(InputBuffer* input_buffer, Statement* statement) {
  ...
  // strtokでスペースごとに分ける
  // strtokを2回目以降呼び出す際は第一引数をNULLにする
  // strtokは先頭の文字のアドレスを返す
